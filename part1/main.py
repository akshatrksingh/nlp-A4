...
def create_augmented_dataloader(args, dataset):
    ...
    # 1. Sample 5k random training examples
    original_train = dataset["train"]
    aug_source = original_train.shuffle(seed=42).select(range(5000))

    # 2. Apply your custom transformation
    aug_transformed = aug_source.map(custom_transform, load_from_cache_file=False)

    # 3. Tokenize transformed examples
    aug_transformed_tok = aug_transformed.map(
        tokenize_function,
        batched=True,
    ...
    # Prepare for PyTorch
    aug_transformed_tok = aug_transformed_tok.remove_columns(["text"])
    aug_transformed_tok = aug_transformed_tok.rename_column("label", "labels")
    aug_transformed_tok.set_format("torch")

    # 4. Tokenize full original training data
    full_train_tok = dataset.map(
        tokenize_function,
        batched=True,
        load_from_cache_file=False
    )["train"]

    full_train_tok = full_train_tok.remove_columns(["text"])
    full_train_tok = full_train_tok.rename_column("label", "labels")
    full_train_tok.set_format("torch")

    # 5. Concatenate datasets
    from datasets import concatenate_datasets
    combined = concatenate_datasets([full_train_tok, aug_transformed_tok])
