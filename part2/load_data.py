...
BOS_ID = PAD_IDX  # I use pad as BOS here

MAX_INPUT_LEN = 256
MAX_OUTPUT_LEN = 256  # long enough for these SQL queries
    ...
    def process_data(self, data_folder, split, tokenizer):
        if split == "train":
            ...
            for q, s in zip(nl_list, sql_list):
                ...
                # encoder side: prefix the question with a task hint
            ...
            for q in nl_list:
                ...
                enc = tokenizer(
                    src_text, padding=False, truncation=True, max_length=MAX_INPUT_LEN, return_tensors=None
                )["input_ids"]
...
def normal_collate_fn(batch):
    """
    Collate function for train/dev. It pads encoder/decoder sequences
    and builds masks and shifted decoder inputs.
    """
    # encoder side
    ...
    # decoder side (teacher forcing with shift-right)
    ...
    for dec in dec_list:
        ...
        if len(tgt) > 1:
            ...
            inp = torch.tensor([BOS_ID], dtype=torch.long) 
...
def test_collate_fn(batch):
    """
    Collate for test-time generation.
    Only returns encoder ids, encoder mask and the first decoder token.
    """
    enc_list = [item["enc_ids"] for item in batch]
    encoder_ids = pad_sequence(enc_list, batch_first=True, padding_value=PAD_IDX)
    encoder_mask = (encoder_ids != PAD_IDX).long()
